...
static irqreturn_t pcie_accel_emu_irq_handler(int irq, void *data)
{
	...
	if (vector == -1) {
		dev_warn(&pemu_dev->pdev->dev, "%s: unhandled irq = %d dev = %d", __func__, irq,
			 pemu_dev->major);
		return IRQ_NONE;
	}
...
static int pcie_accel_emu_irq_enable_msi(struct pcie_accel_emu_dev *pemu_dev)
{
	...
	dev_dbg(&pemu_dev->pdev->dev, "%s: requesting %d MSI vectors", __func__, msi_vecs_req);
	...
	for (int i = 0; i < PCIEMU_HW_IRQ_CNT; i++) {
		...
		err = request_irq(pemu_dev->irq.irq_nums[i], pcie_accel_emu_irq_handler, 0,
				  "pcie_accel_emu_irq_handler", pemu_dev);
		if (err) {
			dev_err(&pemu_dev->pdev->dev, "%s: failed to request irq %d (%d)", __func__,
...
void pcie_accel_emu_irq_disable(struct pcie_accel_emu_dev *pemu_dev)
{
	...
	for (int i = 0; i < PCIEMU_HW_IRQ_CNT; i++) {
		if (pemu_dev->irq.irq_nums[i] > 0) {
			...
			dev_dbg(&pemu_dev->pdev->dev, "%s: freed IRQ %d", __func__,
